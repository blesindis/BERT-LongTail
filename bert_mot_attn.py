...
CENTER_PATH = os.path.join(CENTER_MODEL_PATH, 'centers-4-ffn.pth')
STORE_FOLDER = "0112-mot-inhib-attn-wiki103(256)-(update center)-(ffn)"
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(train_loader):
                ...
                for j in range(config.num_hidden_layers):
                    ...
                    # h_ = center_model.bert.encoders.layers[j].attention.dense(h_)
                    h_ = center_model.bert.encoders.layers[j].attention(hidden_states, batch['attention_mask'])
                    h_ = center_model.bert.encoders.layers[j].ffn(h_)
