...
# CENTER_MODEL_PATH = "outputs/0126-bert-mix2data-bs24-3epoch/checkpoint-20000"
...
STORE_FOLDER = "0128-momoe(switch)-wiki103-bs24-3epoch"
    ...
    if NEED_CENTER:
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(train_loader):
                ...
                for j in range(config.num_hidden_layers):
                    attn_ = center_model.bert.encoders.layers[j].attention(hidden_states, batch['attention_mask'])
                    hidden_states = center_model.bert.encoders.layers[j](hidden_states, batch['attention_mask'])
                    if i == 0:
                        attn_outputs.append(attn_.to('cpu'))
                        layer_outputs.append(hidden_states.to('cpu'))
                    else:
                        attn_outputs[j] = torch.cat([attn_outputs[j], attn_.to('cpu')], dim=0)                
        ...
        for j, layer_output in enumerate(layer_outputs):                 
            ...
            for e in range(NUM_EXPERTS):
                attn_expert = attn_outputs[j][cluster_indexes[e], :, :]
                _, ffn_center = cluster_kmeans(attn_expert.view(-1, 768), NUM_FFN_EXPERTS)
                ffn_centers.append(ffn_center)
            layer_ffn_centers['layer' + str(j)] = ffn_centers
            layer_cluster_centers['layer' + str(j)] = cluster_centers
                    
        torch.save(layer_cluster_centers, CENTER_PATH)
        torch.save(layer_ffn_centers, FFN_CENTER_PATH)
        del center_model
    
    centers = load_layer_data(CENTER_PATH)
    ffn_centers = load_layer_data(FFN_CENTER_PATH)
    model = base_models.BertWithMoMoE(config, centers, ffn_centers)
