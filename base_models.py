...
class BertWithMoMoShareCommonAttn(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoShareCommonAttn, self).__init__()
        self.config = config        
        self.bert = BertMoMoShareCommonAttnModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoShareCommonAttnLarge(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoShareCommonAttnLarge, self).__init__()
        self.config = config        
        self.bert = BertMoMoShareCommonAttnLargeModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoShareCommonAttnLargeNew(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoShareCommonAttnLargeNew, self).__init__()
        self.config = config        
        self.bert = BertMoMoShareCommonAttnLargeNewModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoCombineAttnLinear(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoCombineAttnLinear, self).__init__()
        self.config = config        
        self.bert = BertMoMoCombineAttnLinearModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoCommonAttnLoRA(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoCommonAttnLoRA, self).__init__()
        self.config = config        
        self.bert = BertMoMoCommonAttnLoRAModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoDynamicCenter(nn.Module):
    def __init__(self, config):
        super(BertWithMoMoDynamicCenter, self).__init__()
        self.config = config        
        self.bert = BertMoMoDynamicCenterModel(config)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoDynamicDistribution(nn.Module):
    def __init__(self, config):
...
class BertWithMoMoModelRouterCommonLinGate(nn.Module):
    def __init__(self, config, centers):
        super(BertWithMoMoModelRouterCommonLinGate, self).__init__()
        self.config = config        
        self.bert = BertMoMoModelRouterCommonLinGateModel(config, centers)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
