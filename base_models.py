...
from transformer.SwitchDS import SwitchDSModel
...
from transformer.MoMoEncoderGatingMCR import BertMoMoEncoderGatingMCRModel
from transformer.MoMoAutoEncoderGating import BertMoMoAutoEncoderGatingModel
...
class SwitchDS(nn.Module):
    def __init__(self, config):
        super(SwitchDS, self).__init__()
        self.config = config
        self.bert = SwitchDSModel(config)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
...
class BertWithMoMoEncoderGatingMCR(nn.Module):
    def __init__(self, config):
        super(BertWithMoMoEncoderGatingMCR, self).__init__()
        self.config = config        
        self.bert = BertMoMoEncoderGatingMCRModel(config)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
    ...
    def forward(self, input_ids, attention_mask, labels):
        output, mcr_loss = self.bert(input_ids, attention_mask)
        scores = self.head(output)
        mlm_loss = self.criterion(scores.view(-1, self.config.vocab_size), labels.view(-1)) # scores should be of size (num_words, vocab_size)        
        return mlm_loss, mcr_loss, scores
    
    
class BertWithMoMoAutoEncoderGating(nn.Module):
    def __init__(self, config):
        super(BertWithMoMoAutoEncoderGating, self).__init__()
        self.config = config        
        self.bert = BertMoMoAutoEncoderGatingModel(config)
        self.head = BertOnlyMLMHead(config)
        self.criterion = nn.CrossEntropyLoss() 
        self.apply(self._init_weights)
    ...
    def forward(self, input_ids, attention_mask, labels):
        output, mse_loss = self.bert(input_ids, attention_mask)
        scores = self.head(output)
        mlm_loss = self.criterion(scores.view(-1, self.config.vocab_size), labels.view(-1)) # scores should be of size (num_words, vocab_size)        
        return mlm_loss, mse_loss, scores
