...
class Experts(nn.Module):
    ...
    def routing(self, hidden_states, attention_mask):
        ...
        for expert in self.experts:
            experts_outputs.append(expert(hidden_states, attention_mask).view(-1, 768))
    ...
    def routing_forward(self, hidden_states, attention_mask):
        ...
        experts_outputs = []
        for expert in self.experts:
            # att_output = expert.attention(hidden_states, attention_mask)
            # att_output = expert.LayerNorm(att_output)
            # ffn_output = expert.ffn(att_output)
            # experts_outputs.append(ffn_output)
            experts_outputs.append(expert(hidden_states, attention_mask).view(-1, 768))
        for i in range(hidden_states.shape[0]):
            output_i = [experts_outputs[j][i].mean(dim=0) for j in range(self.config.num_experts)]
            dists_i_list = [torch.dist(output_i[j], self.centers[j]) for j in range(self.config.num_experts)]
...
class TokenExperts(nn.Module):
    ...
    def center_update(self, update_data):
        ...
        for i, data in enumerate(update_data):
            if len(data) > 0:
                ...
                # self.centers[i] = (self.centers[i] + delta) / 2
                # self.centers[i] = delta
                self.centers[i] = self.centers[i] * 0.9 + delta * 0.1
    ...
    def routing(self, hidden_states, attention_mask):
        ...
        experts_outputs = []
        for expert in self.experts:
            experts_outputs.append(expert(hidden_states))
        for i in range(hidden_states.shape[0]):
            output_i = [experts_outputs[j][i].mean(dim=1) for j in range(self.config.num_experts)]
            dists_i_list = [torch.dist(output_i[j], self.centers[j]) for j in range(self.config.num_experts)]
            dists_i = torch.cat(dists_i_list)
            selected = torch.argmin(dists_i)
            cluster_list[selected].append(i)
    ...
    def routing_forward(self, hidden_states, attention_mask):
        ...
        experts_outputs = []
        for expert in self.experts:
            experts_outputs.append(expert(hidden_states, attention_mask))
            # experts_outputs.append(expert(hidden_states, attention_mask).view(-1, 768))
        for i in range(hidden_states.shape[0]):
            output_i = [experts_outputs[j][i].mean(dim=0) for j in range(self.config.num_experts)]            
            dists_i_list = [torch.dist(output_i[j], self.centers[j]) for j in range(self.config.num_experts)]
    ...
    def forward(self, hidden_states, attention_mask):
        output = hidden_states.new_zeros(hidden_states.shape)        
        cluster_list = self.routing(hidden_states.detach(), attention_mask.detach())
        for i in range(self.config.num_experts):
            output[cluster_list[i], :, :] = self.experts[i](hidden_states[cluster_list[i], :, :], attention_mask[cluster_list[i], :])
        # output = self.routing_forward(hidden_states, attention_mask)
        return output
...
# Bert with moe
class BertMOTModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMOTModel, self).__init__()
        self.embeddings = Embeddings(config)
        self.layers = BertMOT(config, centers)
