...
class ExpertFFNUnique(nn.Module):
    def __init__(self, config):
        ...
        if config.unique_moe_lora:
            self.ffn = SwitchFeedForwardLoRA(config, lora_dim=config.unique_moe_lora_dim, n_experts=config.num_experts, drop_tokens=config.unique_moe_drop_tokens)
        else:
            self.ffn = SwitchFeedForward(config)
...
class MoMoShareLayer(nn.Module):
    def __init__(self, config):
        ...
        self.unique_ffn = ExpertFFNUnique(config)
    ...
    def forward(self, hidden_states, attention_mask, cluster_list):
        ...
        unique_ffn_output = self.unique_ffn(att_output)
...
class BertMoMoTModelRouterMTLPlatteModel(nn.Module):
    def __init__(self, config):
        super(BertMoMoTModelRouterMTLPlatteModel, self).__init__()
...
class BertWithMoMoTModelRouterMTLPlatte(nn.Module):
    def __init__(self, config):
        super(BertWithMoMoTModelRouterMTLPlatte, self).__init__()
        self.config = config        
        self.bert = BertMoMoTModelRouterMTLPlatteModel(config)
