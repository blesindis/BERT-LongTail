...
class MoMoShareLayerFirst(nn.Module):
    def __init__(self, config):
        super(MoMoShareLayerFirst, self).__init__()
        self.config = config
        self.lora_dim = 128
        self.n_experts = 2
...
class BertMoMoShare(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [MoMoShareLayerFirst(config)] +
            [MoMoShareLayer(config) for _ in range(config.num_hidden_layers - 1)]
        )        
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        for i, layer in enumerate(self.layers):
            if i == 0:
                hidden_states, cluster_list = layer(hidden_states, attention_mask)
            else:
                hidden_states = layer(hidden_states, attention_mask, cluster_list)
        return hidden_states


class BertMoMoTSwitchEmbModel(nn.Module):
    def __init__(self, config):
        super(BertMoMoTSwitchEmbModel, self).__init__()
