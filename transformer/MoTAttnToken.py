...
class Experts(nn.Module):
    ...
    def routing(self, hidden_states, attention_mask):
        ...
        for expert in self.experts:
            ...
            experts_outputs.append(h_.view(-1, 768))
        for i in range(hidden_states.shape[0] * hidden_states.shape[1]):
    ...
    def forward(self, hidden_states, attention_mask):
        # h_ = hidden_states.view(-1, hidden_states.shape[-1])
        # output = hidden_states.new_zeros(h_.shape)        
        # cluster_list = self.routing(hidden_states.detach(), attention_mask.detach())
        # print(len(cluster_list[0]), len(cluster_list[1]))
        # for i in range(self.config.num_experts):
        #     output[cluster_list[i], :] = self.experts[i](h_[cluster_list[i], :].view(1, -1, 768), attention_mask[0][0]).view(-1, 768)
...
# Bert with moe
class BertMOTAttnTokenModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMOTAttnTokenModel, self).__init__()
