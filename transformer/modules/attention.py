...
class OrthoMHSA(nn.Module):
    def __init__(self, config):
        super(OrthoMHSA, self).__init__()
        self.config = config
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
        self.num_attention_heads = config.num_attention_heads
    ...
    def generate_random_hadamard_product_torch(self, n):
        # Generate a Hadamard matrix of size n using SciPy
        H_np = hadamard(n) / n
        H = torch.from_numpy(H_np).float()  # Convert to PyTorch tensor

        # Generate a random ±1 diagonal matrix
        random_diagonal = np.random.choice([-1, 1], size=n)
        D_np = np.diag(random_diagonal)
        D = torch.from_numpy(D_np).float()  # Convert to PyTorch tensor

        # Multiply the Hadamard matrix by the random diagonal matrix
        return torch.mm(H, D).to('cuda')
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask):
        ...
        #     for h in range(self.num_attention_heads)
        # ])
        single_modulation_matrix = self.generate_random_hadamard_product_torch(self.attention_head_size)
        self.modulation_matrices = [single_modulation_matrix for _ in range(self.num_attention_heads)]
        print(single_modulation_matrix)
        
        q, k, v = tuple(rearrange(qkv, 'h b n (k d) -> k b h n d', k=3))
        
        # Apply the modulation matrix to Q and K for each head
        q = torch.stack([torch.matmul(q[:, i, :, :], self.modulation_matrices[i]) for i in range(self.num_attention_heads)], dim=1)
        k = torch.stack([torch.matmul(k[:, i, :, :], self.modulation_matrices[i]) for i in range(self.num_attention_heads)], dim=1)
...
class OrthoAttention(nn.Module):
    def __init__(self, config):
        super(OrthoAttention, self).__init__()
        self.self = OrthoMHSA(config) # split multi-head
        # self.self = SelfAttention(config)
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
