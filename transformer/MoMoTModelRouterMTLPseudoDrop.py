...
class MoMoShareLayer(nn.Module):
    def __init__(self, config):
        ...
        self.capacity_attn = config.capacity_attn
    ...
    def routing(self, hidden_states):
        ...
        capacity = int(self.capacity_attn * len(h_encoded) / self.n_experts)
        dropped = []
        if self.drop_attn:
            for i in range(self.n_experts):
                if len(cluster_list[i]) > capacity:
                    ...
                    dropped.append(cluster_list[i][capacity:])
                    cluster_list[i] = cluster_list[i][:capacity]
        

        return cluster_list, route_prob_max, dropped
    ...
    def forward(self, hidden_states, attention_mask, cluster_list):
        ...
        if self.step <= self.warm_up_step:
            ...
            common_attn_output = self.common_attn(hidden_states, attention_mask)
            unique_attn_output = hidden_states.new_zeros(hidden_states.shape)
            router_cluster_list, route_prob_max, dropped = self.routing(hidden_states)            
            for i in range(self.n_experts):
            ...
            if dropped:
                ...
                unique_attn_output[dropped, :, :] = hidden_states[dropped, :, :]
...
class BertMoMoTModelRouterMTLPseudoDropModel(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertWithMoMoTModelRouterMTLPseudoDrop(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config        
        self.bert = BertMoMoTModelRouterMTLPseudoDropModel(config)
