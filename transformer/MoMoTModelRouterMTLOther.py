...
class BertMoMoShare(nn.Module):
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers // 2):
            layers += [TransformerEncoder(config), MoMoShareLayer(config)]
        self.layers = nn.ModuleList(layers)
        # self.layers = nn.ModuleList(
        #     [nn.ModuleList([TransformerEncoder(config), MoMoShareLayer(config)]) for _ in range(config.num_hidden_layers // 2)]
        # )
...
class BertMoMoTModelRouterMTLOtherModel(nn.Module):
    def __init__(self, config):
        super(BertMoMoTModelRouterMTLOtherModel, self).__init__()
...
class BertWithMoMoTModelRouterMTLOther(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config        
        self.bert = BertMoMoTModelRouterMTLOtherModel(config)
