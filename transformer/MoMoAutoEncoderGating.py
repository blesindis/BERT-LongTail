...
class ExpertAttention(nn.Module):
    def __init__(self, config):
        ...
        #     nn.Linear(self.lora_dim, config.hidden_size),
        #     nn.Linear(config.hidden_size, config.hidden_size * config.seq_len)
        # )
        """2"""
        self.encoder = nn.Linear(config.hidden_size, self.lora_dim)
        self.decoder = nn.Linear(self.lora_dim, config.hidden_size)
    ...
    def routing(self, hidden_states):
        ...
        # h_decoded = self.decoder(h_encoded)
        # mse_loss = self.mse_loss(h_decoded, hidden_states.reshape(batch_size, seq_len * d_model))
        """2"""
        h_encoded = self.encoder(hidden_states.mean(dim=1))
        h_decoded = self.decoder(h_encoded)
        mse_loss = self.mse_loss(h_decoded, hidden_states.mean(dim=1))
        ...
        return cluster_list, route_prob_max, mse_loss
    
    def forward(self, hidden_states, attention_mask):
        output = hidden_states.new_zeros(hidden_states.shape)
        
        cluster_list, route_prob_max, mse_loss = self.routing(hidden_states)
...
class MoMoShareLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        """Post Norm"""
        att_output, mse_loss = self.attention(hidden_states, attention_mask)
        ffn_output = self.ffn(att_output)
        ffn_output = self.dropout(ffn_output)
        output = self.LayerNorm(att_output + ffn_output)        
...
class BertMoMoShare(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        for i, layer in enumerate(self.layers):
            hidden_states, mse_loss = layer(hidden_states, attention_mask)
            total_mse_loss += mse_loss
        return hidden_states, total_mse_loss


class BertMoMoAutoEncoderGatingModel(nn.Module):
    def __init__(self, config):
        super(BertMoMoAutoEncoderGatingModel, self).__init__()
        self.embeddings = Embeddings(config)
        self.layers = BertMoMoShare(config)
        
    def forward(self, input_ids, attention_mask):
        embeddings = self.embeddings(input_ids)
        outputs, mse_loss = self.layers(embeddings, attention_mask)
