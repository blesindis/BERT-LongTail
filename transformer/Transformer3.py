...
class TransformerEncoderCombineResidual(nn.Module):
    def __init__(self, config) -> None:
        super(TransformerEncoderCombineResidual, self).__init__()
    ...
    def forward(self, hidden_states, attention_mask) -> torch.Tensor:
        ...
        att_output = self.attention(hidden_states, attention_mask)
        ffn_output = self.ffn(att_output)
        output = self.LayerNorm(hidden_states + ffn_output)   
        return output
    
    
class TransformerEncodersCombineResidual(nn.Module):
    def __init__(self, config):
        super(TransformerEncodersCombineResidual, self).__init__()
        self.config = config
        self.layers = nn.ModuleList([TransformerEncoderCombineResidual(config) for _ in range(config.num_hidden_layers)])
...
# tradition bert
class BertModelCombineResidualPostNorm(nn.Module):
    def __init__(self, config):
        super(BertModelCombineResidualPostNorm, self).__init__()
        self.embeddings = Embeddings(config)
        self.encoders = TransformerEncodersCombineResidual(config)
