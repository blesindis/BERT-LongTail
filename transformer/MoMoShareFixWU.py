...
class ExpertAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, expert_index, warmup):
...
class MoMoShareLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, expert_index, warmup):
        ...
        """Post Norm"""
        att_output = self.attention(hidden_states, attention_mask, expert_index, warmup)
        ffn_output = self.ffn(att_output)
        ffn_output = self.dropout(ffn_output)
        output = self.LayerNorm(att_output + ffn_output)        
...
class BertMoMoShare(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, expert_index, warmup):
        for i, layer in enumerate(self.layers):
            hidden_states = layer(hidden_states, attention_mask, expert_index, warmup)
        return hidden_states


class BertMoMoShareFixModelWU(nn.Module):
    def __init__(self, config, centers):
        super(BertMoMoShareFixModelWU, self).__init__()
        self.embeddings = Embeddings(config)
        self.layers = BertMoMoShare(config, centers)
        
    def forward(self, input_ids, attention_mask, expert_index, warmup):
        embeddings = self.embeddings(input_ids)
        outputs = self.layers(embeddings, attention_mask, expert_index, warmup)
        return outputs