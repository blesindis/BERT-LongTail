...
class SwitchLayers(nn.Module):
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers // 2):
            layers += [TransformerEncoder(config), SwitchEncoder(config)]
...
class SwitchOtherModel(nn.Module):
    def __init__(self, config):
        super(SwitchOtherModel, self).__init__()
...
class BertSwitchOther(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.bert = SwitchOtherModel(config)
