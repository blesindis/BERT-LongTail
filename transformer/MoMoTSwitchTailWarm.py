...
class TailLayer(nn.Module):
    def __init__(self, config):
        ...
        # WARM UP
        self.switch_warm = nn.Linear(config.hidden_size, 1)
        self.warm_step = 2000
        self.step = 0
    ...
    def forward(self, hidden_states, attention_mask):
        if self.step < self.warm_step:
            route_prob = self.softmax(self.switch_warm(hidden_states.mean(dim=1)))
            route_prob_max, routes = torch.max(route_prob, dim=-1)
            route_prob = route_prob_max
            attn_output = self.attention[0](hidden_states, attention_mask)
            attn_output *= (route_prob / route_prob.detach()).unsqueeze(-1).unsqueeze(-1).expand_as(hidden_states)
            
            ffn_output = self.ffn[0](attn_output)
            ffn_output *= (route_prob / route_prob.detach()).unsqueeze(-1).unsqueeze(-1).expand_as(hidden_states)
            ...
            if self.step == self.warm_step:
                self.step += 1
                # Copy weights
                self.switch.weight.data[0, :] = self.switch_warm.weight.data.squeeze()
                # Copy bias for the first expert
                self.switch.bias.data[0] = self.switch_warm.bias.data
                # print("SUCCESS")
...
class BertMoMoTSwitchTailWarmModel(nn.Module):
    def __init__(self, config):
        super(BertMoMoTSwitchTailWarmModel, self).__init__()
...
class BertWithMoMoTSwitchTailWarm(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config        
        self.bert = BertMoMoTSwitchTailWarmModel(config)
