...
class ExpertAttention(nn.Module):
    def __init__(self, config, centers):
        ...
        self.config = config
        self.n_experts = 2
        self.experts = nn.ModuleList([LoRAAttention(config, lora_dim=256) for i in range(self.n_experts)])
        self.common_expert = LoRAAttention(config, lora_dim=256)
...
class BertMoMoCommonAttnLoRAModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMoMoCommonAttnLoRAModel, self).__init__()
