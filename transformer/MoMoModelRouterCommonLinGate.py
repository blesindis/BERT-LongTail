...
from transformer.modules.attention import Attention, SmallAttention
from transformer.modules.feedforward import FeedForward, SwitchFeedForward
    
    
class ExpertAttention(nn.Module):
    def __init__(self, config, centers, common_dim, unique_dim):
        super().__init__()
        self.config = config
        self.common_dim = common_dim
        self.unique_dim = unique_dim
        self.n_experts = 2
        self.experts = nn.ModuleList([SmallAttention(config, in_dim=unique_dim) for i in range(self.n_experts)])
        self.common_experts = SmallAttention(config, in_dim=common_dim)
    ...
    def forward(self, hidden_states, attention_mask, routing_states):
        ...
        for i in range(self.n_experts):                        
            output[cluster_list[i], :, :] = self.experts[i](hidden_states[cluster_list[i], :, self.common_dim:], attention_mask[cluster_list[i], :])
        output += self.common_experts(hidden_states[:,:,:self.common_dim], attention_mask)
        return output
...
class MoMoShareLayer(nn.Module):
    def __init__(self, config, centers):
        ...
        self.config = config
        self.attention = ExpertAttention(config, centers, common_dim=512, unique_dim=256)
        self.lingate = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(self, hidden_states, attention_mask, routing_states):
        ...
        """Post Norm"""
        dim_output = self.lingate(hidden_states)
        att_output = self.attention(dim_output, attention_mask, routing_states)
        ffn_output = self.ffn(att_output)
        ffn_output = self.dropout(ffn_output)
        output = self.LayerNorm(att_output + ffn_output)        
...
class BertMoMoModelRouterCommonLinGateModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMoMoModelRouterCommonLinGateModel, self).__init__()
