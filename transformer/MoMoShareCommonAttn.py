...
class ExpertAttention(nn.Module):
    def __init__(self, config, centers):
        ...
        self.config = config
        self.n_experts = 2
        self.experts = nn.ModuleList([Attention(config) for i in range(self.n_experts + 1)])
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        common_output = self.experts[-1](hidden_states, attention_mask)
...
class BertMoMoShareCommonAttnModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMoMoShareCommonAttnModel, self).__init__()
