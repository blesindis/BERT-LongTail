...
from transformer.modules.transformer import Embeddings, Attention, FeedForward, MHSA
...
class ExpertAttention(nn.Module):
    def __init__(self, config, centers):
        ...
        self.config = config
        self.n_experts = 2
        self.experts = nn.ModuleList([MHSA(config) for i in range(self.n_experts)])
...
class MoMoShareLayer(nn.Module):
    def __init__(self, config, centers):
        ...
        self.att_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.att_LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, hidden_states, attention_mask, routing_states):
        ...
        att_output = self.dense(att_output)
        att_output = self.att_dropout(att_output)
        att_output = self.att_LayerNorm(att_output)
        ffn_output = self.ffn(att_output)
        ffn_output = self.dropout(ffn_output)
        output = self.LayerNorm(att_output + ffn_output)        
...
class BertMoMoCombineAttnLinearModel(nn.Module):
    def __init__(self, config, centers):
        super(BertMoMoCombineAttnLinearModel, self).__init__()
