...
class LePuWiki():
    ...
    def __init__(self, config):
        ...
        train_legal, train_pub, train_wiki = Subset(datasets[0]['train'], range(100000)), Subset(datasets[1]['train'], range(100000)), Subset(datasets[2]['train'], range(300000))
...
class WikiSST():
    ...
    def __init__(self, config):
        ...
        paths = [
            '/home/mychen/ER_TextSpeech/BERT/data/datasets/tokenized/wikipedia',
            '/home/mychen/ER_TextSpeech/BERT/data/datasets_pure_ft/tokenized/SST-2'
        ]
        ...
        val_wiki, val_sst = Subset(datasets[0]['validation'], range(1600)), datasets[1]['validation']
        train_wiki, train_sst = Subset(datasets[0]['train'], range(3000000)), datasets[1]['train']
        
        val_data = ConcatDataset([val_wiki, val_sst])
        train_data = ConcatDataset([train_wiki, train_sst])
        ...
        self.val_loader = DataLoader(val_data, batch_size=self.batch_size, shuffle=False, collate_fn=data_collator)
        self.val_loader_wiki = DataLoader(val_wiki, batch_size=self.batch_size, shuffle=False, collate_fn=data_collator)
        self.val_loader_sst = DataLoader(val_sst, batch_size=self.batch_size, shuffle=False, collate_fn=data_collator)
...
class Wikipedia():
    ...
    def __init__(self, config):
        ...
        if not config.preprocessed:
            self.preprocess(config, path)
        lm_datasets = load_from_disk(path)
        lm_datasets_train = torch.utils.data.Subset(lm_datasets['train'], range(3000000))
        lm_datasets_val = torch.utils.data.Subset(lm_datasets['validation'], range(1600))
        seed = 45
        torch.manual_seed(seed)
        np.random.seed(seed)
        ...
        self.train_loader = DataLoader(lm_datasets_train, batch_size=self.batch_size, shuffle=True, collate_fn=data_collator)
        self.val_loader = DataLoader(lm_datasets_val, batch_size=self.batch_size, shuffle=False, collate_fn=data_collator)
        self.unshuffled_train_loader = DataLoader(lm_datasets_train, batch_size=self.batch_size, shuffle=False, collate_fn=data_collator)
        self.train_set = lm_datasets_train
        # self.test_loader = DataLoader(lm_datasets['test'], batch_size=self.batch_size, shuffle=False, collate_fn=data_collator) 
