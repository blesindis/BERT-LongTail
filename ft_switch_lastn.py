...
from transformer.FT_SwitchLastN import BertSwitchLastNForSequenceClassification
...
# folder paths
LOAD_FOLDER = "wikisst(128)300w-bs64-epoch1-lr3-moe_lastn(6)/checkpoint-47928"
LOAD_PATH = os.path.join('outputs', LOAD_FOLDER)
STORE_FOLDER = "ft-sst-wikisst(128)300w-bs64-epoch1-lr3-moe_lastn(6)"
...
def main():
    ...
    base_model = BertSwitchLastN(config)
    checkpoint = torch.load(os.path.join(LOAD_PATH, 'pytorch_model.bin'))
    base_model.load_state_dict(checkpoint)
    
    # Incorporate into finetune model
    model = BertSwitchLastNForSequenceClassification(config, num_labels=2)
