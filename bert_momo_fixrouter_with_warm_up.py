...
STORE_FOLDER = "0227-momoshare_(fixrouter-wu)-mix2-bs64-5epoch"
...
def validate(model, val_loader, accelerator, expert_index, warmup):
    losses = []    
    for i, batch in enumerate(val_loader):    
        with torch.no_grad():
            loss, _ = model(**batch, expert_index=expert_index, warmup=warmup)
...
def main():
    ...
    model = base_models.BertWithMoMoShareFixWU(config, centers)
    ...
    for epoch in range(num_epochs):
        ...
        for i, batches in enumerate(zip(*train_loaders)):     
            ...
            for index, batch in enumerate(batches):
                ...
                loss_d, _ = model(**batch, expert_index=index, warmup=warmup)
                loss += loss_d
            loss /= NUM_DATASETS
            ...
            if step % 100 == 0:
                ...
                for e in range(NUM_DATASETS):
                    loss_valid += validate(model, val_loaders[e], accelerator, expert_index=e, warmup=warmup)
