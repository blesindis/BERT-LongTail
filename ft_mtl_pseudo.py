...
from transformer.MoMoTModelRouterMTLPseudoLoRA import BertWithMoMoTModelRouterMTLPseudoLoRA, MoMoShareLayer
from transformer.FT_MTLPseudoLoRA import BertMTLPseudoLoRAForSequenceClassification
import matplotlib.pyplot as plt

# Local imports
from Dataset_ft import SST2, Overruling, GAD
...
LOAD_FOLDER = "wiki(128)300w-bs64-1epoch-lr3-momot_model_router_mtl_pseudo_lora(lora-u-128, 6-4, warm-5000)_layer(full)_router5000/checkpoint-46875"
LOAD_PATH = os.path.join('outputs', LOAD_FOLDER)
STORE_FOLDER = "ft-gad-wiki(128)300w-bs64-1epoch-lr3-momot_model_router_mtl_pseudo_lora(lora-u-128, 6-4, warm-5000)_layer(full)_router5000"
...
def main():
    ...
    base_model = BertWithMoMoTModelRouterMTLPseudoLoRA(config)
    ...
    model = BertMTLPseudoLoRAForSequenceClassification(config, num_labels=2)
    copy_parameters(base_model.bert, model.bert)
    
    for l in range(config.num_hidden_layers):
        if isinstance(model.bert.layers.layers[l], MoMoShareLayer):
            model.bert.layers.layers[l].step = 10000
