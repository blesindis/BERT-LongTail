...
from transformer.FT_MTLPlatte import BertMTLPlatteForSequenceClassification
...
LOAD_FOLDER = "LPW(128)(280+10+10)w-bs64-1epoch-lr3-momot_model_router_mtl_platte(same-param, common-full)_layer(6)_router5000/checkpoint-46875"
LOAD_PATH = os.path.join('outputs', LOAD_FOLDER)
STORE_FOLDER = "ft-SST-LPW(128)(280+10+10)w-bs64-1epoch-lr3-momot_model_router_mtl_platte(same-param, common-full)_layer(6)_router5000"
...
def main():
    ...
    base_model = BertWithMoMoTModelRouterMTLPlatte(config)
    checkpoint = torch.load(os.path.join(LOAD_PATH, 'pytorch_model.bin'))
    base_model.load_state_dict(checkpoint)
    
    # Incorporate into finetune model
    model = BertMTLPlatteForSequenceClassification(config, num_labels=2)
