...
# folder paths
STORE_FOLDER = "0301-momo_dynamic_center-mix2-bs64-5epoch-lr3-wu5000"
...
def main():
    ...
    model = base_models.BertWithMoMoDynamicCenter(config)
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):                      
            ...
            if step % 5000 == 0:
                layer_centers = {}
                for l in range(config.num_hidden_layers):
                    centers = model.bert.layers.layers[l].attention.centers
                    layer_centers['layer' + str(l)] = centers
                accelerator.save_state(os.path.join(STORE_PATH, 'checkpoint-' + str(step)))
                torch.save(centers, os.path.join(STORE_PATH, 'checkpoint-' + str(step), 'centers.pth'))
            step += 1
