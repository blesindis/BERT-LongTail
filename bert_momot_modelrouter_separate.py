...
from transformer.MoMoTModelRouterSeparate import BertWithMoMoTModelRouterSeparate
...
CENTER_PATH = os.path.join(CENTER_MODEL_PATH, 'centers-2-momoe-transformer-lastlayer-separate.pth')
STORE_FOLDER = "bert(128)300w-bs64-1epoch-lr3-momot_model_router_separate_lora128_normalmoe_layer(0)_router10000"
STORE_PATH = os.path.join('outputs', STORE_FOLDER)
CONFIG_PATH = 'config/model_router_last.json'
...
def validate(model, val_loader, accelerator, center_model, centers):
    ...
    for i, batch in enumerate(val_loader):    
        with torch.no_grad():
            cluster_list = [[] for _ in range(NUM_EXPERTS + 1)]
            ...
            cluster_list = [torch.eq(min_indices, i).nonzero(as_tuple=True)[0] for i in range(NUM_EXPERTS + 1)]   
...
def main():
    ...
    if NEED_CENTER:
        ...
        common_centers = last_layer_outputs.mean(dim=1).mean(dim=0).to('cpu').numpy().reshape(1,768)
        total_centers = np.concatenate([common_centers, cluster_centers], axis=0)
        
        layer_cluster_centers['layer_last'] = total_centers        
    ...
    model = BertWithMoMoTModelRouterSeparate(config)
