...
from transformer.MoMoTModelRouterMTLPseudoDrop import BertWithMoMoTModelRouterMTLPseudoDrop, MoMoShareLayer
...
STORE_FOLDER = "wiki(128)300w-bs64-1epoch-lr3-momot_model_router_mtl_pseudo_drop(lora-u-128, 6-4, warm-5000)_layer(full)_router5000"
STORE_PATH = os.path.join('outputs', STORE_FOLDER)
CONFIG_PATH = 'config/mtl_drop.json'
...
def main():
    ...
    model = BertWithMoMoTModelRouterMTLPseudoDrop(config)
    ...
    for epoch in range(num_epochs):
        ...
        losses = []        
        for i, batch in enumerate(train_loader):    
            if model.bert.layers.layers[-1].step <= model.bert.layers.layers[-1].warm_up_step:
                with torch.no_grad():          
                    cluster_list = [[] for _ in range(NUM_EXPERTS)]
