...
# folder paths
CENTER_MODEL_PATH = "outputs/0113-mot-inhib-lora(256)-warmup-wiki103(256)/checkpoint-5000"
CENTER_PATH = os.path.join(CENTER_MODEL_PATH, 'centers-4-attn.pth')
STORE_FOLDER = "0113-mot-inhib-attn-lora-wiki103(256)"
...
def main():
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(train_loader):
                ...
                for j in range(config.num_hidden_layers):
                    ...
                    """PostNorm"""
                    h_ = center_model.bert.layers.layers[j].attention.self(hidden_states, batch['attention_mask'])
                    h_ = center_model.bert.layers.layers[j].attention.dense(h_)
    ...
    model = base_models.BertWithMoTAttnLoRA(config, centers)
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):                      
            ...
            if step % 100 == 0:
                ...
                loss_train = torch.mean(torch.cat(losses)[:3200])
